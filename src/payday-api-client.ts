...
export class PaydayAPIClient {
	...
	constructor(config: PaydayAPIClientConfiguration) {
		...
		if (!config.environment) throw new Error('Missing environment in configuration.');
		if (!PaydayAPIClientBaseURL[config.environment]) throw new Error(`Invalid environment in configuration: ${config.environment}`);
